...
for INPUT in ${0%/*}/regression_tests/*.csv; do
	...
	while IFS= read -r input_line; do
		...
		OUTPUT_AND_RESULT=$(valgrind -v --leak-check=yes ${0%/*}/rtl_number ${RTL_CMD_IN} $2>1)
		...
		if 	[ "_$OUTPUT_AND_RESULT" != "_" ] &&
			[ "_" == "_$(echo $OUTPUT_AND_RESULT | grep ERROR)" ]; then
				...
				TEST_OUTPUT=$(valgrind -v --leak-check=yes ${0%/*}/rtl_number ${test_output_args})
				...
				[ "1'b1" == "$TEST_OUTPUT" ]; then
					...
					FAILURE_COUNT=$((FAILURE_COUNT+1))
					echo -e "-X- FAILED == $TEST_LABEL\t valgrind -v --leak-check=yes ${0%/*}/rtl_number ${RTL_CMD_IN}\t CMP=${TEST_OUTPUT} Output:<$OUTPUT_AND_RESULT> != <$EXPECTED_RESULT>"
				fi
		else
			FAILURE_COUNT=$((FAILURE_COUNT+1))
			echo -e "-X- FAILED == $TEST_LABEL\t  valgrind -v --leak-check=yes ${0%/*}/rtl_number ${RTL_CMD_IN}\t  Output:<$OUTPUT_AND_RESULT> != <$EXPECTED_RESULT>"
		fi
