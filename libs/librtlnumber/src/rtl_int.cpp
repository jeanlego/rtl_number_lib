...
{
	/* TODO: Pass in VerilogBits */VerilogBits VerilogBits_a(a_in);
	/* TODO: Pass in VerilogBits */VerilogBits VerilogBits_b(b_in);

	bool neg_a = (VerilogBits_a.is_negative());
	bool neg_b = (VerilogBits_b.is_negative());
	...
	/* TODO: Use Same VerilogBits as above */VerilogBits VerilogBits_a_pad(a);
	/* TODO: Use Same VerilogBits as above */VerilogBits VerilogBits_b_pad(b);
	const char pad_a = VerilogBits_a_pad.get_padding_bit();
	const char pad_b = VerilogBits_b_pad.get_padding_bit();
...
{
	int std_length = std::max(a.size(), b.size());
	/* TODO: Pass in VerilogBits */VerilogBits VerilogBits_a(a);
	/* TODO: Pass in VerilogBits */VerilogBits VerilogBits_b(b);
	const char pad_a = VerilogBits_a.get_padding_bit();
	const char pad_b = VerilogBits_b.get_padding_bit();
...
std::string V_CASE_EQUAL(const std::string& a,const std::string& b)
{
	...
	/* TODO: Pass in VerilogBits */VerilogBits VerilogBits_a(a);
	/* TODO: Pass in VerilogBits */VerilogBits VerilogBits_b(b);
	char last_a = VerilogBits_a.get_padding_bit();
	char last_b = VerilogBits_b.get_padding_bit(); 
...
std::string V_SIGNED_SHIFT_RIGHT(const std::string& a_in, const std::string& b)
{
	...
	shift_op(a, -1 * bits_str_to_int(value), VerilogBits_a.get_padding_bit());
	return a;
}
...
std::string V_POWER(const std::string& a,const std::string& b)
{
	...
	{
		if(VerilogBits_a.is_negative()) 	// even
			return "0111";
		else				//	odd
			return "1000";
	}
